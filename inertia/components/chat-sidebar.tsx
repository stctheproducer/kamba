...
const mockUserPrompts: CustomPrompt[] = [
  {
    ...
    systemPrompt: "You are a senior code reviewer. Analyze code for best practices, security issues, and improvements.",
  ...
  },
  {
    ...
    systemPrompt: "You are a meeting assistant. Summarize meeting notes and extract key action items and decisions.",
...
const mockAdminPrompts: CustomPrompt[] = [
  {
    ...
    systemPrompt: "You are an expert programmer. Assist with coding tasks and explain concepts clearly.",
    recommendedModelId: "gpt-4o",
    category: "Development",
    isPublic: true,
    upvotes: 152,
  ...
  },
  {
    ...
    systemPrompt: "You are a creative writing assistant. Help brainstorm ideas and write engaging narratives.",
...
export function ChatSidebar({
  ...
  const handleDeleteChat = (id: string) => {
    ...
    if (currentConversationId === id) {
      onSelectConversation(localConversations.length > 1 ? localConversations.find((c) => c.id !== id)!.id : null)
    }
  }
  ...
  const handleSaveEdit = (id: string) => {
    setLocalConversations(localConversations.map((conv) => (conv.id === id ? { ...conv, title: editingTitle } : conv)))
    setEditingId(null)
  }
